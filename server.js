...
var Server = function() {
    ...
    io.on("connect", function(socket) {
        ...
        var init = function() {
            console.log("User connected "+ world.users_peeked);
            world.entities[world.users_peeked++] = client;
            io.sockets.emit('update', world);
        }

        socket.on("disconnect", function() {
            console.log("User disconnected "+ world.users_peeked);
        ...
        socket.on("command", function(string) {
            ...
            switch (data[0]) {
                ...
                case "h": {
                    world.entities[client.id].h = string.substring((string.length - (string.length - data[0].length)));
                    io.sockets.emit('update', world);   /* update name to everyone */
                }
                    break;
